...
int follow_trail(vector<string>& grid, int y, int x){
  // Just remove the visited set and you get every single trail
  if(grid[y][x] == '9'){
    return 1;
  }
  int result = 0;
  for(const auto& dir : directions){
    ...
    result += follow_trail(grid,dy,dx);
  }
  return result;
}
