  ...
  for(int i = 0; i < (int)mat.size()-1;i++){
    for(int j = 0; j < (int)mat.size()-1;j++){
      if(mat[i][j] == 'A'){
        ...
        bool right_diagonal = search(mat, i - 1, j - 1, Vec2(1,1),  needle)  || search(mat, i - 1,  j - 1,  Vec2(1,1),  needle_backwards);
        bool left_diagonal =  search(mat, i - 1, j + 1, Vec2(1,-1), needle)  || search(mat, i - 1,  j + 1,  Vec2(1,-1), needle_backwards);
